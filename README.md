...
注意点:
- フラグが未作成・不明な `status` の場合は **公開扱い（後方互換）** になります。ただし `src/featureFlags.js` の `KNOWN_FLAGS` に登録された組み込み機能は例外で、未作成時は `defaultStatus` で動作します。先行公開したい機能は、コードを書く前に必ずフラグを `βテスト中` で作るか、`KNOWN_FLAGS` に登録してください。
- 現在組み込みのβ対象機能（キー・未作成時のデフォルト）:
  - `web-push`（プッシュ通知）: `beta`
  - `absence-request`（公欠申請）: `beta`
  公開するには管理画面「βテスト」で同じキーのドキュメントを作成し、状態を `公開` にします。
- 承認側（`approve.html` のメール承認・`teacher.html` の先生ダッシュボード）はβ期間中も動かす必要があるため、βゲートの対象外です。生徒側の申請フォーム・ナビとプッシュ通知カードのみが対象です。
