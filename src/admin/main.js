...
import {
  ...
  orderBy, query, where, serverTimestamp, Timestamp,
...
async function saveModal() {
  ...
  try {
    ...
    // お知らせ (Issue #37): 必須チェック＋新規作成時に createdAt を付与
    if (editingType === 'announcements') {
      if (!data.title) { showToast('タイトルを入力してください'); btn.disabled = false; return }
      if (!data.body) { showToast('本文を入力してください'); btn.disabled = false; return }
      if (!editingId) data.createdAt = serverTimestamp()
    }
    if (editingId) {
      await updateDoc(doc(db, editingType, editingId), data)
      showToast(editingType === 'announcements' ? 'お知らせを更新しました' : '更新しました')
    } else {
...
openModal = async function(type, id = null) {
  await _origOpenModal(type, id)
